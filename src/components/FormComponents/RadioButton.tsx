...
export interface RadioButtonProps extends BaseFormControlProps {
  options: Option[]
}
...
const RadioButton = ({ label, id, name, options, className, ...rest }: RadioButtonProps) => {
  return (
    <fieldset className={`flex flex-col space-y-2 mb-4 ${className || ''}`}>
      <legend className="font-bold text-xl text-gray-600">
        {label}
      </legend>
            ...
            return options.map((option: Option) => (
              <div className="flex items-center" key={option.key}>
                <input
                  ...
                  className="mr-1.5"
                />
                <label htmlFor={`${id}-${option.value}`} className="radio-item">
